...
def get_target_price(ticker):
    ...
    df = pyupbit.get_ohlcv(ticker, interval="minute10", count=4) #현재시간 10분 단위 30분전 데이터의 고점 저점을 기준으로 계산            *
...
def macd(df: pd.DataFrame, macd_short:int=12, macd_long:int=26, macd_signal:int=9):
    ...
    df["macd_up"]=(df["MACD_signal"].iloc[197]<df["MACD"].iloc[197])and(df["MACD_signal"].iloc[198]<df["MACD"].iloc[198])and(df["MACD_signal"].iloc[199]<df["MACD"].iloc[199])and(df["MACD_signal"].iloc[193]>df["MACD"].iloc[193])
