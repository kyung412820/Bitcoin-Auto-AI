...
def macd(df: pd.DataFrame, macd_short:int=12, macd_long:int=26, macd_signal:int=9):
    ...
    for i in range(0,3):
        if(df["MACD"][197-i]<df["MACD_signal"][197-i])and(df["MACD"][198-i]>df["MACD_signal"][198-i]):   
            result="매수"
        else:
            result="보류"
    return result
...
def stock_rsi(df: pd.DataFrame, p1, k1, d1):
        ...
        for i in range(0,3):
                if(stochrsi_K[197-i]<=20)and(stochrsi_K[198-i]>=20):
                    return "매수"
...
def AI(target, name, set_time):
    ...
    if(sell_target=="0"):
        ...
        get_ma_result=get_ma(Heikin_Ashi_result,200)
        for i in range(0,3):
            if(Heikin_Ashi_result['close'][197-i]>max_check):
                max_check=Heikin_Ashi_result['close'][197-i]
        if (Heikin_Ashi_result['close'][198]>max_check)and(Heikin_Ashi_result['open'][198]==Heikin_Ashi_result['low'][198]):
            timing="매수"
        else:
            timing="보류"
        print(stock_rsi_result,get_ma_result,timing)
        ADX_result=ADX(df)
        #OBV_result=OBV(df)
        ...
        if (stock_rsi_result=="매수"and macd_current=="매수" and rsi_current=="매수" and ADX_result=="매수"): #get_ma = 이평선 120, 60의 계산값
            krw = get_balance("KRW")
            if krw > 5000:
    ...
    elif ((current_price>=(buy_price*1.075))or(current_price<=(buy_price*0.995))):
