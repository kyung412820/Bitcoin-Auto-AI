...
def get_target_price(ticker):
    ...
    df = pyupbit.get_ohlcv(ticker, interval="day", count=11) #현재시간 10분 단위 30분전 데이터의 고점 저점을 기준으로 계산            *
...
def AI(target, name, set_time):
    ...
    df = pyupbit.get_ohlcv(target, interval="day")
    cci_current=CCI(df)
    ...
    if (macd_current=="매수"and target_price=="매수"):
        krw = get_balance("KRW")
        if krw > 5000:
