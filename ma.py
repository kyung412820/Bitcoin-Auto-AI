...
def macd(df: pd.DataFrame, macd_short:int=12, macd_long:int=26, macd_signal:int=9):
    ...
    if (df["MACD_signal"][198]>df["MACD_signal"][197]):
        result="매수"
    else:
        result="보류"
    return result
...
def get_ma(df, n1, n2): #이평선 계산 n1(60 이평선)-n2(120 이평선)
    ...
    if ((time1[198]>time2[198])and(time1[197]<time2[197])): #60이평선 - 120이평선의 값이 계속 증가하고 이평선60이 120보다 높을 경우라면
        return "up"
    elif ((time1[196]<time1[197])and(time1[198]<time1[197])):
        return "down"
...
def stock_rsi(df: pd.DataFrame, p1, k1, d1):
        ...
        if(stochrsi_K[197]<=0.3 and (stochrsi_K[197]<stochrsi_K[198]) and (stochrsi_K[196]>stochrsi_K[197])):
            return "매수"
        elif(stochrsi_K[197]>=0.7 and (stochrsi_K[197]>stochrsi_K[198]) and (stochrsi_K[196]<stochrsi_K[197])):
            return "매도"
        else:
            return "보류"
...
def AI(target, name, set_time):
    ...
    if (get_ma(df, 60, 120)=="up"): #get_ma = 이평선 120, 60의 계산값
        krw = get_balance("KRW")
        if krw > 5000:
