...
def get_target_price(ticker):
    ...
    df = pyupbit.get_ohlcv(ticker, interval="minute10", count=21) #현재시간 10분 단위 30분전 데이터의 고점 저점을 기준으로 계산            *
...
def predict_price(ticker):
    ...
    hour_ch, minute10=time_changer(now,30)#현시각보다 30분 뒤의 시각을 계산
...
def macd(df: pd.DataFrame, macd_short:int=12, macd_long:int=26, macd_signal:int=9):
    ...
    df["macd_down"]=(df["Indicator"].iloc[199]>0 and df["Indicator"].iloc[198]>0 and df["Indicator"].iloc[197]>0)
    #1단위 전부터 현재까지 macd가 macd_signal보다 작은지
    df["macd_up"]=(df["Indicator"].iloc[199]>0 and df["Indicator"].iloc[198]>0 and df["Indicator"].iloc[197]>0 and df["Indicator"].iloc[193]<0)
...
def AI(target, name, set_time, i):
    ...
    if (target_price=="매수" and macd_current=="매수" and current_price < predicted_close_price[i]):
        krw = get_balance("KRW")
        if krw > 5000:
...
while True:
    try:
        ...
        AI("KRW-AAVE","AAVE",now,4)
        AI("KRW-NEO","NEO",now,5)
        AI("KRW-WAVES","WAVES",now,6)
        AI("KRW-LSK","LSK",now,7)
